          ...
          <v-stepper editable v-model="currentStep" :items="steps" hide-actions>
            ...
            <template v-slot:item.2>
              ...
              <v-card class="mt-4 pa-4" variant="text">
                ...
                <div class="fence-visualization mb-4">
                  <svg width="100%" height="400" :viewBox="svgViewBox" style="background-color: #f5f5f5; border-radius: 8px">
                    ...
                    <line x1="0" :y1="centerY" x2="100%" :y2="centerY" stroke="#9E9E9E" stroke-width="1" stroke-dasharray="5,5" />
                    <line :x1="centerX" y1="0" :x2="centerX" y2="100%" stroke="#9E9E9E" stroke-width="1" stroke-dasharray="5,5" />

  
                    <path :d="fencePath" fill="none" stroke="#1976D2" stroke-width="4" stroke-linejoin="round" />


                    <template v-for="(section, i) in fenceSections" :key="'section-' + i">
                      <line
                        v-if="i < fencePoints.length - 1"
                        :x1="fencePoints[i].x"
                        :y1="fencePoints[i].y"
                        :x2="fencePoints[i + 1].x"
                        :y2="fencePoints[i + 1].y"
                        stroke="#424242"
                        stroke-width="8"
                    ...
                    <text
                      v-for="(section, i) in fenceSections"
                      v-if="i < fencePoints.length - 1"
                      :key="'length-' + i"
                      :x="(fencePoints[i].x + fencePoints[i + 1].x) / 2"
                      :y="(fencePoints[i].y + fencePoints[i + 1].y) / 2 - 15"
                      font-size="11"
                      fill="#4CAF50"
                      text-anchor="middle"
                      font-weight="bold"
                    ...
                    <g v-for="(section, i) in fenceSections" :key="'angle-' + i">
                      <template v-if="i > 0 && section.connectionType === 'Угол'">
                        <path :d="getAngleArc(i)" fill="none" stroke="#FF9800" stroke-width="2" stroke-dasharray="3,2" />
                        <circle :cx="fencePoints[i].x" :cy="fencePoints[i].y" r="4" fill="#FF9800" />
                        <text
                          :x="fencePoints[i].x + (getSectionAngle(i) > 90 ? -30 : 30)"
                          :y="fencePoints[i].y - 15"
                          font-size="12"
                          fill="#FF9800"
                          text-anchor="middle"
                          font-weight="bold"
                    ...
                    <g class="legend" :transform="`translate(${legendX}, ${legendY})`">
                      <rect x="0" y="0" width="180" height="90" fill="white" stroke="#E0E0E0" rx="4" fill-opacity="0.9" />
                      <text x="10" y="20" font-size="12" fill="#424242" font-weight="500">Легенда:</text>
                      <circle cx="15" cy="35" r="4" fill="#1976D2" stroke="#FFF" stroke-width="1" />
                      <text x="25" y="38" font-size="11" fill="#424242">Точка соединения</text>
                      <line x1="10" y1="50" x2="20" y2="50" stroke="#1976D2" stroke-width="3" />
                      <text x="25" y="53" font-size="11" fill="#424242">Участок забора</text>
                      <path d="M10,65 A5,5 0 0,1 20,65" fill="none" stroke="#FF9800" stroke-width="2" stroke-dasharray="3,2" />
                      <text x="25" y="68" font-size="11" fill="#424242">Угол поворота</text>
                    </g>
                  </svg>
                </div>
...
import { ralColors, ralColorMap } from "@/data/ralColors";
...
const getLegendTransform = computed(() => {
  const [minX, minY, width, height] = svgViewBox.value.split(" ").map(Number);
  const legendWidth = 180;
  const legendHeight = 90;
  const padding = 10;

  return `translate(${width - legendWidth - padding}, ${height - legendHeight - padding})`;
});
...
const initialSection = {
  ...
  claddingStep: 5, // Значение по умолчанию в см
};
...
const svgViewBox = computed(() => {
  ...
  // Добавляем отступы (больше снизу для легенды)
  const padding = 50;
  minX -= padding;
  minY -= padding * 2; // Больше отступ сверху
  maxX += padding;
  maxY += padding * 3; // Больше отступ снизу для легенды
...
/* Уменьшаем отступы между полями ввода и чекбоксами */
.v-input {
  margin-top: 0;
  margin-bottom: 8px;
}
...
@media (max-width: 768px) {
  ...
  .legend {
    transform: scale(0.8) translate(calc(100% - 150px), calc(100% - 80px)) !important;
  }
...
<!-- <template>
  <v-app>
    <v-main>
      <v-container class="py-8">
        <v-card class="mx-auto" max-width="1000" elevation="8" rounded="lg">
          <v-card-title class="text-h4 pa-6 primary white--text">
            <v-icon large left color="white">mdi-fence</v-icon>
            Конфигуратор забора
          </v-card-title>

          <v-stepper editable v-model="currentStep" :items="steps" hide-actions color="primary" class="elevation-0">
            <template v-slot:item.1>
              <v-card variant="flat" class="pa-4 pa-md-6">
                <div class="text-h6 mb-6 primary--text">Контактная информация</div>
                <v-form class="pt-2" v-model="isStep1Valid" @submit.prevent="nextStep">
                  <v-text-field
                  ...
                  <div class="d-flex justify-end mt-6">
                    <v-btn color="primary" size="large" @click="nextStep" :disabled="!isStep1Valid">
                      Далее
            ...
            <template v-slot:item.2>
              <div class="text-h6 mb-4 primary--text">Участки забора</div>

              <v-expansion-panels multiple v-model="expandedSections" class="mb-6">
                <v-expansion-panel v-for="(section, index) in fenceSections" :key="index" class="mb-4 elevation-2">
                  <v-expansion-panel-title class="text-subtitle-1">
                    Участок {{ index + 1 }}
                    <v-chip v-if="section.type" small class="ml-2" color="primary">
                      {{ section.type }}
                    </v-chip>
                    <v-chip v-if="section.length" small class="ml-2"> {{ section.length }} м </v-chip>
                  </v-expansion-panel-title>
                  <v-expansion-panel-text>
                    <v-card variant="text" class="pa-2">
                      <div class="d-flex justify-space-between align-center mb-4">
                        <h3 class="text-h6">Участок {{ index + 1 }}</h3>
                        <v-btn v-if="fenceSections.length > 1" icon color="error" @click="removeSection(index)" variant="text">
                          <v-icon>mdi-delete</v-icon>
                        </v-btn>
                      </div>
                      ...
                      <v-row>
                        ...
                        <v-col cols="12" class="pt-0" md="6" v-if="section.coatingType === 'Полимерное (RAL)'">
                          <v-select
                            v-model="section.ralColor"
                            :items="ralColors"
                            label="Цвет RAL"
              ...
              <div class="d-flex flex-wrap justify-center mb-6">
                <v-btn color="primary" variant="outlined" @click="addSection" prepend-icon="mdi-plus"> Добавить участок </v-btn>
              </div>

              <v-card class="mt-4 pa-4" variant="outlined">
                <h3 class="text-h6 mb-4 primary--text">Схема забора</h3>
                <div class="fence-visualization mb-4">
                  <svg
                    width="100%"
                    height="300"
                  ...
                  >
                    <line x1="0" y1="0" x2="100%" y2="0" stroke="#ddd" stroke-width="1" stroke-dasharray="5,5" />
                    <line x1="0" y1="0" x2="0" y2="100%" stroke="#ddd" stroke-width="1" stroke-dasharray="5,5" />

                    <path :d="fencePath" fill="none" stroke="#1976D2" stroke-width="3" />

                    <template v-for="(point, idx) in fencePoints" :key="idx">
                      <circle :cx="point.x" :cy="point.y" r="5" fill="#FF5252" />
                      <text :x="point.x + 8" :y="point.y + 5" font-size="12" fill="#333">
                        {{ idx + 1 }}
                      </text>
                    </template>

                    <template v-for="(section, idx) in fenceSections" :key="'length-' + idx">
                      <text
                        v-if="idx < fencePoints.length - 1"
                        :x="(fencePoints[idx].x + fencePoints[idx + 1].x) / 2"
                        :y="(fencePoints[idx].y + fencePoints[idx + 1].y) / 2 - 10"
                        font-size="11"
                        fill="#4CAF50"
                        text-anchor="middle"
                      >
                    ...
                    <template v-for="(section, idx) in fenceSections" :key="'angle-' + idx">
                      <g v-if="idx > 0 && section.connectionType === 'Угол'">
                        <path :d="getAngleArc(idx)" fill="none" stroke="#FF9800" stroke-width="1.5" />
                        <text
                          :x="fencePoints[idx].x + (getSectionAngle(idx) > 90 ? -25 : 25)"
                          :y="fencePoints[idx].y - 10"
                          font-size="11"
                          fill="#FF9800"
                          text-anchor="middle"
                        >
                ...
                <div class="pa-3" style="background-color: #f5f5f5; border-radius: 4px">
                  <p><strong>Общая длина:</strong> {{ totalLength.toFixed(2) }} м</p>
                  <p><strong>Количество углов:</strong> {{ angleCount }}</p>
                  <p v-if="closeFence && fenceSections.length >= 3"><strong>Площадь участка:</strong> {{ area.toFixed(2) }} м²</p>
                </div>
              </v-card>
              ...
              <div class="d-flex justify-space-between mt-6">
                <v-btn color="secondary" variant="outlined" @click="prevStep" prepend-icon="mdi-arrow-left"> Назад </v-btn>
                <v-btn color="primary" @click="nextStep" append-icon="mdi-arrow-right"> Далее </v-btn>
              </div>
            </template>

            <template v-slot:item.3>
              <div class="text-h6 mb-6 primary--text">Монтаж и доставка</div>
              ...
              <v-card variant="outlined" class="pa-4 mb-6">
                ...
                ></v-checkbox>
                <v-checkbox
                  v-model="installation.needsInstallation"
                  label="Нужен монтаж"
                  class="mt-0 mb-2"
                  density="compact"
                  color="primary"
            ...
            <template v-slot:item.4>
              <div class="text-h6 mb-6 primary--text">Подтверждение заказа</div>

              <v-card variant="outlined" class="pa-4 mb-6">
                <h3 class="text-h6 mb-4 primary--text">Контактная информация</h3>
                ...
                <h3 class="text-h6 mb-4 primary--text">Участки забора</h3>
                <v-divider class="mb-4"></v-divider>
                <div v-for="(section, index) in fenceSections" :key="index" class="mb-4">
                  ...
                  <div class="info-grid">
                    <div>
                      ...
                      <p>- Длина: {{ section.length }} м</p>
                      <p>- Высота: {{ section.height }} м</p>
                      <p v-if="section.type.includes('Штакетник')">
                        - Обшивка: {{ section.claddingType }}, шаг: {{ section.claddingStep }} см
                      </p>
                    </div>
                    <div>
                      <p>
                  ...
                  <div v-if="section.hasGates" class="mt-2">
                    <p>- Ворота:</p>
                    <div class="info-grid">
                      <div>
                        <p>- Тип: {{ section.gatesType }}</p>
                  ...
                  <v-divider v-if="index < fenceSections.length - 1" class="my-3"></v-divider>
                </div>

                <v-divider class="my-4"></v-divider>

                <h3 class="text-h6 mb-4 primary--text">Монтаж и доставка</h3>
                <v-divider class="mb-4"></v-divider>
                <div class="info-grid">
                  <div>
                    <p><strong>Доставка:</strong> {{ installation.needsDelivery ? "Да" : "Нет" }}</p>
                ...
                <h3 class="text-h6 mb-4 primary--text">Итоговая информация</h3>
                <v-divider class="mb-4"></v-divider>
                <div class="info-grid">
                  <div>
                    <p><strong>Общая длина забора:</strong> {{ totalLength.toFixed(2) }} м</p>
              ...
              <div class="d-flex justify-space-between mt-6">
                <v-btn color="secondary" variant="outlined" @click="prevStep" prepend-icon="mdi-arrow-left"> Назад </v-btn>
                <v-btn color="success" @click="submitForm" size="large" append-icon="mdi-send"> Отправить заявку </v-btn>
              </div>
            </template>
          </v-stepper>
        </v-card>
...
const fencePoints = computed(() => {
  ...
  fenceSections.value.forEach((section, index) => {
    if (index === 0) {
      const length = (section.length || 0) * 5;
      points.push({ x: points[0].x + length, y: points[0].y });
      return;
    }
...
const area = computed(() => {
  ...
  const points = fencePoints.value.slice(0, -1);
...
const addSection = () => {
  fenceSections.value.push({ ...initialSection });
  expandedSections.value.push(fenceSections.value.length - 1);
};

const removeSection = (index) => {
  if (fenceSections.value.length > 1) {
    fenceSections.value.splice(index, 1);
    expandedSections.value = expandedSections.value.map((i) => (i > index ? i - 1 : i)).filter((i) => i !== index);
  }
};
...
.v-stepper-item--active .v-stepper-item__title {
  font-weight: 600;
  color: #1976d2;
}
</style> -->
